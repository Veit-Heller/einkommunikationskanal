...
export default function VorgaengePage() {
  ...
  const filtered = vorgaenge.filter(v =>
    filter === "alle" ? true : v.status === filter
  );
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-slate-300 mt-1">
              {filter === "offen" ? "Alle Vorgänge sind erledigt 🎉" : "Noch keine Einträge in dieser Kategorie"}
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {/* Status badge */}
                          <span className={`flex-shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${cfg.bg} ${cfg.color}`}>
